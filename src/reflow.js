...
function joinDecision(leftChar, rightChar, opts) {
  ...
  if (lCJK || rCJK) {
    // 至少一侧是 CJK
    if (opts.cjkLatinSpace && lCJK !== rCJK) return { sep: ' ', stripHyphen: false };
...
function joinText(acc, frag, opts) {
  ...
  return left + sep + frag.replace(/^\s+/, '');
}
...
function buildLineText(items, H, opts) {
  ...
  for (const it of sorted) {
    ...
    text = text + sep + it.str.replace(/^\s+/, '');
...
export function reflow(items, options = {}) {
  ...
  // 样本足够时用行距中位数作“正常行距”；样本太少时退回按字高估计（≈1.6 字高）
  const normalGap = posGaps.length >= 4 ? median(posGaps) : H * 1.6;
